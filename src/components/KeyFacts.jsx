...
const AnimatedCounter = ({ from = 0, to, suffix = '+', duration = 1.8 }) => {
...
const AnimatedDecimalCounter = ({ to = 1.5, suffix = 'K+', duration = 1.8 }) => {
...
/* ══════════════════════════════════════════════════════
   Scroll-linked 3D Perspective Tilt Cards
   Left card  : rotateY  -55 → 0, scale 0.78 → 1, z -260 → 0
   Right card : rotateY   55 → 0, scale 0.78 → 1, z -260 → 0
   Centre card stays flat (z-index higher, no perspective tilt).
══════════════════════════════════════════════════════ */
const usePerspectiveTilt = (sectionRef, direction) => {
  ...
  // Ease the raw scroll progress with a gentle spring
...
const KeyFacts = () => {
  ...
  // Centre card: simple scale-pop reveal (no perspective tilt)
    ...
    <section id="keyfacts" className="kf__section" ref={sectionRef}>
      <div className="kf__container">
        ...
        <div className="kf__cards-grid kf__perspective-stage">
          ...
          {/* ── Card 1: Featured & Awards (LEFT — tilts in from left) ── */}
          <motion.div
          ...
          {/* ── Card 2: Projects Completed (CENTRE — flat, always in front) ── */}
          <motion.div
          ...
          {/* ── Card 3: Our Team Members (RIGHT — tilts in from right) ── */}
          <motion.div
            className="kf__card kf__card--dark kf__card--side"
            style={{
